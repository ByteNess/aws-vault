...
An alternative to manually supplying the prompt driver as a CLI argument to `aws-vault` is setting the [`mfa_process`]({{% relref "/docs/config" %}}#mfa_process) parameter in your `.aws/config` for the profiles that should use a YubiKey to generate tokens. Example:
